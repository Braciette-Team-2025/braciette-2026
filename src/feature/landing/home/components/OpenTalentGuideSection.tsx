  ...
  return (
    <section className="flex flex-col items-center justify-center px-4 lg:px-25 gap-5 md:gap-7 xl:gap-10 md:h-screen">
      <div className="flex flex-col w-full gap-3">
        ...
        <p className="text-xs md:text-2xl xl:text-4xl font-the-seasons">
          Cari tahu cara mendaftar dan tunjukkan potensi terbaikmu.
        </p>
      </div>
      <div className="grid grid-cols-3 gap-16 md:gap-24 xl:gap-48">
        {SUBMISSION_GUIDE_DATA.map((data) => {
          return (
            <SubmissionTextbox key={data.id} id={data.id} desc={data.desc} />
          );
        })}
      </div>
      <div className="w-32 md:w-64 xl:w-100">
